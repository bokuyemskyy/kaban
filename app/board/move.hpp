...
struct State {
   public:
    ...
    [[nodiscard]] constexpr Castling castling() const { return (m_value >> CASTLING_SHIFT) & CASTLING_MASK; }

    [[nodiscard]] constexpr EnPassant en_passant() const { return (m_value >> EN_PASSANT_SHIFT) & EN_PASSANT_MASK; }

    [[nodiscard]] constexpr uint8_t halfmoves() const { return (m_value >> HALFMOVES_SHIFT) & HALFMOVES_MASK; }

    constexpr void setCaptured(Piece captured) { m_value |= ((captured.value() & CAPTURED_MASK) << CAPTURED_SHIFT); }
    [[nodiscard]] constexpr Piece captured() const { return (m_value >> CAPTURED_SHIFT) & CAPTURED_MASK; }

   private:
    uint16_t m_value;

    static constexpr int CASTLING_BITS   = 4;
    static constexpr int EN_PASSANT_BITS = 4;
    static constexpr int HALFMOVES_BITS  = 6;
    static constexpr int CAPTURED_BITS   = 4;

    static constexpr int CASTLING_SHIFT   = 0;
    static constexpr int EN_PASSANT_SHIFT = CASTLING_SHIFT + CASTLING_BITS;
    static constexpr int HALFMOVES_SHIFT  = EN_PASSANT_SHIFT + EN_PASSANT_BITS;
    static constexpr int CAPTURED_SHIFT   = HALFMOVES_SHIFT + HALFMOVES_BITS;

    static constexpr uint32_t CASTLING_MASK   = (1 << CASTLING_BITS) - 1;
    static constexpr uint32_t EN_PASSANT_MASK = (1 << EN_PASSANT_BITS) - 1;
    static constexpr uint32_t HALFMOVES_MASK  = (1 << HALFMOVES_BITS) - 1;
    static constexpr uint32_t CAPTURED_MASK   = (1 << CAPTURED_BITS) - 1;
};
